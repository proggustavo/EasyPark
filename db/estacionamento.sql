...
UPDATE USUARIO SET TIPO = 1 WHERE IDUSUARIO = 2;

SELECT * FROM USUARIO;

UPDATE USUARIO SET LOGIN = 'gustavo', SENHA = 'asdf1234', TIPO = 1 WHERE IDUSUARIO = 0;

SELECT COUNT(IDVAGA) FROM VAGA WHERE SITUACAO = 'LIVRE' and TIPO = 'ESPECIAL'ORDER BY DISTANCIA LIMIT 1;
...
SELECT PAGAR(3);
SELECT DHSAIDA(4);
SELECT DTFINALIZADO(4);

UPDATE VAGA SET TIPO = 'LIVRE', SITUACAO = 'LIVRE' WHERE IDVAGA = 8;

DESC VAGA;
SELECT IDVAGA, DISTANCIA, TIPO, SITUACAO FROM VAGA;
SELECT * FROM TICKET;
SELECT * FROM VAGA;

DELETE FROM VAGA WHERE IDVAGA = 8;
...
BEGIN
        ...
        SET vIDVAGA = pIDVAGA;
			...
			IF vDHENTRADA IS NULL THEN
                ...
                UPDATE VAGA SET SITUACAO = "OCUPADA" WHERE vIDVAGA = IDVAGA;
...
BEGIN
    ...
    DECLARE vHORAS INT;
    DECLARE vVALORTICKET DOUBLE DEFAULT 0.00;
    DECLARE vVALORPRIMEIRAHORA DOUBLE;
    DECLARE vVALORHORA DOUBLE;
    DECLARE vRESULTADO VARCHAR(4500) DEFAULT "";
    SET vVALORTICKET = 0.00;
	...
	SELECT MINUTE(TIMEDIFF(NOW(), DHEMISSAO)) INTO vMINUTOS FROM TICKET WHERE IDVAGA = pIDVAGA AND DTFINALIZADO IS NULL; 
			...
			END IF;	
				IF (vMINUTOS > 0 AND vHORAS <= 10) THEN
					CASE 
						WHEN vMINUTOS = 0 THEN SET vVALORTICKET = vVALORTICKET;
                        ...
                        WHEN vMINUTOS <= 59 THEN SET vVALORTICKET = vVALORTICKET + (vVALORHORA/4) * 4;
                ...
                SET vRESULTADO = CONCAT(vRESULTADO , "VALOR TOTAL = " , vVALORTICKET);
                RETURN vRESULTADO;
...
UPDATE TICKET SET DHPAGAMENTO = NULL WHERE IDTICKET = 5;
DROP FUNCTION PAGAR;
SELECT PAGAR(3);
SELECT * FROM VAGA;
...
SELECT HOUR(TIMEDIFF(NOW(), DHEMISSAO)) FROM TICKET WHERE IDVAGA = 5 AND DTFINALIZADO IS NULL;  

-- ---------------------------------- SAIR VAGA -----------------------------------------

DROP PROCEDURE SP_DHSAIDA;

DELIMITER $$
CREATE FUNCTION DHSAIDA( pIDVAGA INT) RETURNS VARCHAR(4500)
BEGIN
	DECLARE vRESULTADO VARCHAR(4500);
    ...
    IF(pIDVAGA IS NOT NULL) THEN
		...
		SET vRESULTADO = "VAGA LIBERADA!";
    ELSE
		 SET vRESULTADO = "INFORME O NÚMERO DA VAGA";
	END IF;
...
UPDATE TICKET SET DHSAIDA = NULL WHERE IDTICKET = 4;
SELECT * FROM TICKET;
...
DROP PROCEDURE SP_DTFINALIZADO;
DELIMITER $$
CREATE FUNCTION DTFINALIZADO(pIDVAGA INT) RETURNS VARCHAR(4500)
BEGIN
		DECLARE vDHPAGAMENTO DOUBLE;
		DECLARE vIDTICKET INT;
		SELECT IDTICKET INTO vIDTICKET FROM TICKET WHERE IDVAGA = pIDVAGA AND DTFINALIZADO IS NULL;
        
        SELECT DHPAGAMENTO INTO vDHPAGAMENTO FROM TICKET WHERE IDTICKET = vIDTICKET;
		
        IF vDHPAGAMENTO IS NOT NULL THEN
...
SELECT IDTICKET FROM TICKET WHERE IDVAGA = 3 AND DTFINALIZADO IS NULL;
...
SELECT * FROM VW_BI;
DROP VIEW VW_BI;
CREATE VIEW VW_BI AS
SELECT 
	VAGA.IDVAGA,
    VAGA.TIPO,
    FORMAT(AVG(TIMESTAMPDIFF(SECOND, DHEMISSAO, DHENTRADA)), 0) AS 'Tempo médio para entrada na vaga',
	FORMAT(AVG(TIMESTAMPDIFF(SECOND, DHPAGAMENTO, DHSAIDA)), 0) AS 'Tempo para saída da vaga'   
    
FROM TICKET
RIGHT JOIN VAGA ON 
VAGA.IDVAGA = TICKET.IDVAGA
GROUP BY IDVAGA;
...
GROUP BY 
    ...
    SELECT TOTALVAGAS, TIPO FROM VW_VAGASNORMAIS;
